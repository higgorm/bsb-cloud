function clear_clipboard() {
    bootbox.confirm($("#lang_clear_clipboard_confirm").val(), $("#cancel").val(), $("#ok").val(), function(e) {
        e == 1 && $.ajax({type: "POST", url: "ajax_calls.php?action=clear_clipboard", data: {}}).done(function(e) {
            e != "" ? bootbox.alert(e) : $("#clipboard").val("0");
            toggle_clipboard(!1)
        })
    })
}
function copy_cut_clicked(e, t) {
    ...
    if (!e.hasClass("directory"))
        var n = e.find(".rename-file").attr("data-thumb"), r = e.find(".rename-file").attr("data-path");
    else
        var n = e.find(".rename-folder").attr("data-thumb"), r = e.find(".rename-folder").attr("data-path");
    $.ajax({type: "POST", url: "ajax_calls.php?action=copy_cut", data: {path: r, path_thumb: n, sub_action: t}}).done(function(e) {
        if (e != "")
            bootbox.alert(e);
        else {
            $("#clipboard").val("1");
    ...
    bootbox.confirm($("#lang_paste_confirm").val(), $("#cancel").val(), $("#ok").val(), function(t) {
        if (t == 1) {
            if (typeof e != "undefined")
                var n = e.find(".rename-folder").attr("data-path"), r = e.find(".rename-folder").attr("data-thumb");
            else
                var n = $("#sub_folder").val() + $("#fldr_value").val(), r = $("#cur_dir_thumb").val();
            $.ajax({type: "POST", url: "execute.php?action=paste_clipboard", data: {path: n, path_thumb: r}}).done(function(e) {
                if (e != "")
                    bootbox.alert(e);
                else {
                    $("#clipboard").val("0");
...
function drag_n_drop_paste(e, t) {
    if (!e.hasClass("directory"))
        ...
        var n = e.find(".rename-folder").attr("data-thumb"), r = e.find(".rename-folder").attr("data-path");
    $.ajax({type: "POST", url: "ajax_calls.php?action=copy_cut", data: {path: r, path_thumb: n, sub_action: "cut"}}).done(function(e) {
        if (e != "")
            bootbox.alert(e);
        else {
            if (typeof t != "undefined")
                var n = t.find(".rename-folder").attr("data-path"), r = t.find(".rename-folder").attr("data-thumb");
            else
                var n = $("#sub_folder").val() + $("#fldr_value").val(), r = $("#cur_dir_thumb").val();
            $.ajax({type: "POST", url: "execute.php?action=paste_clipboard", data: {path: n, path_thumb: r}}).done(function(e) {
                if (e != "")
...
function toggle_clipboard(e) {
    e == 1 ? $(".paste-here-btn, .clear-clipboard-btn").removeClass("disabled") : $(".paste-here-btn, .clear-clipboard-btn").addClass("disabled")
}
...
function swipe_reaction(e, t, n, r, i) {
    ...
    if ($("#view").val() == 0)
        if (s.attr("toggle") == 1) {
            s.attr("toggle", 0);
            s.animate({top: "0px"}, {queue: !1, duration: 300})
        } else {
            s.attr("toggle", 1);
            s.animate({top: "-30px"}, {queue: !1, duration: 300})
        }
}
function apply(e, t) {
    if ($("#popup").val() == 1)
        ...
        var n = window.parent;
    var r = $("#cur_dir").val(), i = $("#base_url").val(), s = e.substr(0, e.lastIndexOf(".")), o = e.split(".").pop();
    o = o.toLowerCase();
    var u = "", a = new Array("ogg", "mp3", "wav"), f = new Array("mp4", "ogg", "webm");
    if ($.inArray(o, ext_img) > -1)
        u = '<img src="' + i + r + e + '" alt="' + s + '" />';
    else if ($.inArray(o, f) > -1)
...
function apply_none(e, t) {
    ...
    if (n.html() != "" && n.html() !== undefined) {
        ...
        $("#previewAV").modal({backdrop: "static", keyboard: !1});
        n.hasClass("audio") ? $(".body-preview").css("height", "80px") : $(".body-preview").css("height", "345px");
        $.ajax({url: decodeURIComponent(n.attr("data-url")), success: function(e) {
                $(".body-preview").html(e)
            }})
    }
    return
...
function apply_file_duplicate(e, t) {
    ...
    var i = r.find(".download-form"), s = "form" + (new Date).getTime();
    i.attr("id", s);
...
function apply_file_rename(e, t) {
    ...
    var n = e.find("a.link"), r = n.attr("data-file"), i = r.substring(r.lastIndexOf("/") + 1), s = r.substring(r.lastIndexOf(".") + 1);
    n.each(function() {
    ...
    var o = e.find("a.preview"), r = o.attr("data-url");
    ...
    var u = e.find("a.rename-file"), a = e.find("a.delete-file"), f = u.attr("data-path"), l = u.attr("data-thumb"), c = f.replace(i, t + "." + s), h = l.replace(i, t + "." + s);
    u.attr("data-path", c);
    u.attr("data-thumb", h);
    a.attr("data-path", c);
    a.attr("data-thumb", h)
...
function apply_folder_rename(e, t) {
    ...
    e.find("h4 > a").text(t);
    var r = e.find(".folder-link"), i = r.attr("href"), s = $("#fldr_value").val(), o = i.replace("fldr=" + s + encodeURIComponent(n), "fldr=" + s + encodeURIComponent(t));
    r.each(function() {
        $(this).attr("href", o)
    });
    var u = e.find("a.delete-folder"), a = e.find("a.rename-folder"), f = a.attr("data-path"), l = a.attr("data-thumb"), c = f.lastIndexOf("/"), h = f.substr(0, c + 1) + t;
    u.attr("data-path", h);
    a.attr("data-path", h);
    var c = l.lastIndexOf("/"), h = l.substr(0, c + 1) + t;
    u.attr("data-thumb", h);
    a.attr("data-thumb", h)
}
...
function replaceDiacritics(e) {
    var e, t = [/[\300-\306]/g, /[\340-\346]/g, /[\310-\313]/g, /[\350-\353]/g, /[\314-\317]/g, /[\354-\357]/g, /[\322-\330]/g, /[\362-\370]/g, /[\331-\334]/g, /[\371-\374]/g, /[\321]/g, /[\361]/g, /[\307]/g, /[\347]/g], n = ["A", "a", "E", "e", "I", "i", "O", "o", "U", "u", "N", "n", "C", "c"];
...
function execute_action(e, t, n, r, i, s) {
    if (r !== null) {
        r = fix_filename(r);
        $.ajax({type: "POST", url: "execute.php?action=" + e, data: {path: t, path_thumb: n, name: r.replace("/", "")}}).done(function(e) {
            if (e != "") {
                bootbox.alert(e);
                return!1
            }
...
function sortUnorderedList(e, t, n) {
    typeof e == "string" && (e = $(e));
    var r = e.find("li.dir"), i = e.find("li.file"), s = [], o = [], u = [], a = [];
    $.each(r, function(e) {
        var t = $(this), r = t.find(n).val();
        if ($.isNumeric(r)) {
            r = parseFloat(r);
...
function launchEditor(e, t) {
    featherEditor.launch({image: e, url: t});
    return!1
}
var version = "9.4.0", active_contextmenu = !0;
loading_bar && (/MSIE (\d+\.\d+);/.test(navigator.userAgent) ? $(document).ready(function() {
    $("body").queryLoader2({backgroundColor: "none", minimumTime: 100, percentage: !0})
}) : window.addEventListener("DOMContentLoaded", function() {
    $("body").queryLoader2({backgroundColor: "none", minimumTime: 100, percentage: !0})
}));
$(document).ready(function() {
    if (active_contextmenu) {
        $.contextMenu({selector: "figure:not(.back-directory), .list-view2 figure:not(.back-directory)", autoHide: !0, build: function(e) {
                e.addClass("selected");
                var t = {callback: function(t, n) {
                        switch (t) {
                            case"copy_url":
                                var r = "";
                            ...
                            case"unzip":
                                ...
                                $.ajax({type: "POST", url: "ajax_calls.php?action=extract", data: {path: r}}).done(function(e) {
                                    e != "" ? bootbox.alert(e) : window.location.href = $("#refresh").attr("href") + "&" + (new Date).getTime()
                                });
                                break;
                            case"edit_img":
                                var i = e.attr("data-name"), s = $("#base_url_true").val() + $("#cur_dir").val() + i;
                ...
                if ((e.find(".img-precontainer-mini .filetype").hasClass("png") || e.find(".img-precontainer-mini .filetype").hasClass("jpg") || e.find(".img-precontainer-mini .filetype").hasClass("jpeg")) && image_editor) {
                    t.items.edit_img = {name: $("#lang_edit_image").val(), icon: "edit_img", disabled: !1};
                    t.items.copy_url = {name: $("#lang_show_url").val(), icon: "url", disabled: !1}
                }
                if (e.find(".img-precontainer-mini .filetype").hasClass("zip") || e.find(".img-precontainer-mini .filetype").hasClass("tar") || e.find(".img-precontainer-mini .filetype").hasClass("gz"))
                    t.items.unzip = {name: $("#lang_extract").val(), icon: "extract", disabled: !1};
                !e.hasClass("directory") && $("#duplicate").val() == 1 && (t.items.duplicate = {name: $("#lang_duplicate").val(), icon: "duplicate", disabled: !1});
                if (!e.hasClass("directory") && $("#copy_cut_files_allowed").val() == 1) {
                    t.items.copy = {name: $("#lang_copy").val(), icon: "copy", disabled: !1};
                    t.items.cut = {name: $("#lang_cut").val(), icon: "cut", disabled: !1}
                } else if (e.hasClass("directory") && $("#copy_cut_dirs_allowed").val() == 1) {
                    t.items.copy = {name: $("#lang_copy").val(), icon: "copy", disabled: !1};
                    t.items.cut = {name: $("#lang_cut").val(), icon: "cut", disabled: !1}
                }
                $("#clipboard").val() != 0 && !e.hasClass("directory") && (t.items.paste = {name: $("#lang_paste_here").val(), icon: "clipboard-apply", disabled: !1});
                t.items.sep = "----";
                t.items.info = {name: "<strong>" + $("#lang_file_info").val() + "</strong>", disabled: !0};
                t.items.name = {name: e.attr("data-name"), icon: "label", disabled: !0};
                e.attr("data-type") == "img" && (t.items.dimension = {name: e.find(".img-dimension").html(), icon: "dimension", disabled: !0});
    ...
    $("ul.grid").on("click", ".modalAV", function(e) {
        ...
        $.ajax({url: _this.attr("data-url"), success: function(e) {
                $(".body-preview").html(e)
            }})
    });
    ...
    $("#uploader-btn").on("click", function() {
        ...
        $("#iframe-container").html($("<iframe />", {name: "JUpload", id: "uploader_frame", src: "uploader/index.php?path=" + e, frameborder: 0, width: "100%", height: 360}))
    });
    ...
    $(".sorter").on("click", function() {
        ...
        n = !n;
        if (t) {
            $.ajax({url: "ajax_calls.php?action=sort&sort_by=" + _this.attr("data-sort") + "&descending=" + n}).done(function(e) {
            });
    ...
    $("ul.grid").on("click", ".rename-file", function() {
        var e = $(this), t = e.parent().parent().parent(), n = t.find("h4"), r = $.trim(n.text());
    ...
    $(".new-folder").on("click", function() {
        bootbox.prompt($("#insert_folder_name").val(), $("#cancel").val(), $("#ok").val(), function(e) {
            if (e !== null) {
                ...
                var t = $("#sub_folder").val() + $("#fldr_value").val() + e, n = $("#cur_dir_thumb").val() + e;
                $.ajax({type: "POST", url: "execute.php?action=create_folder", data: {path: t, path_thumb: n}}).done(function(e) {
                    setTimeout(function() {
                        window.location.href = $("#refresh").attr("href") + "&" + (new Date).getTime()
                    }, 300)
                })
    ...
    $(".view-controller button").on("click", function() {
        ...
        e.addClass("btn-inverse");
        e.find("i").addClass("icon-white");
        $.ajax({url: "ajax_calls.php?action=view&type=" + e.attr("data-value")}).done(function(e) {
            e != "" && bootbox.alert(e)
        });
    ...
    if (!Modernizr.touch) {
        ...
        $("#help").show();
        $(".box:not(.no-effect)").swipe({swipeLeft: swipe_reaction, swipeRight: swipe_reaction, threshold: 30})
    }
    $(".paste-here-btn").on("click", function() {
        $(this).hasClass("disabled") == 0 && paste_to_this_dir()
    });
    $(".clear-clipboard-btn").on("click", function() {
        $(this).hasClass("disabled") == 0 && clear_clipboard()
    });
    if (!Modernizr.csstransforms) {
        $("figure").on("mouseover", function() {
            $("#view").val() == 0 && $("#main-item-container").hasClass("no-effect-slide") === !1 && $(this).find(".box:not(.no-effect)").animate({top: "-26px"}, {queue: !1, duration: 300})
        });
        $("figure").on("mouseout", function() {
            $("#view").val() == 0 && $(this).find(".box:not(.no-effect)").animate({top: "0px"}, {queue: !1, duration: 300})
        })
    }
    ...
    $("#clipboard").val() == 1 ? toggle_clipboard(!0) : toggle_clipboard(!1);
    $("li.dir, li.file").draggable({revert: !0, distance: 20, cursor: "move", helper: function() {
            ...
            $("#view").val() == 0 && $("#main-item-container").addClass("no-effect-slide")
        }, stop: function() {
            $(this).removeClass("selected");
            $("#view").val() == 0 && $("#main-item-container").removeClass("no-effect-slide")
        }});
    $("li.dir").droppable({accept: "ul.grid li", activeClass: "ui-state-highlight", hoverClass: "ui-state-highlight", drop: function(e, t) {
