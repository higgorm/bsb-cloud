...
tinymce.PluginManager.add("filemanager", function(e) {
    function t(t, n, r, i) {
        ...
        if (typeof e.settings.filemanager_title !== "undefined" && e.settings.filemanager_title) {
            s = e.settings.filemanager_title
        }
        var o = "key";
        ...
        tinymce.activeEditor.windowManager.open({title: s, file: e.settings.external_filemanager_path + "dialog.php?type=" + urltype + "&descending=" + a + u + f + "&lang=" + e.settings.language + "&akey=" + o, width: 860, height: 570, resizable: true, maximizable: true, inline: 1}, {setUrl: function(n) {
                var r = i.document.getElementById(t);
                r.value = e.convertURL(n);
                if ("fireEvent"in r) {
                    r.fireEvent("onchange")
                } else {
                    var s = document.createEvent("HTMLEvents");
                    s.initEvent("change", false, true);
                    r.dispatchEvent(s)
                }
            }})
    }
    tinymce.activeEditor.settings.file_browser_callback = t;
    return false
})